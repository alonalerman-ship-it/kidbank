  ...
  return (
    <div className="flex min-h-[70vh] items-center pb-28">
      <Card className="w-full overflow-hidden bg-surfaceLowest">
        ...
        <p className="mt-3 text-sm font-medium leading-6 text-mutedInk">Enter the family password to see the app.</p>

        <form action="/api/auth/login" method="post" className="mt-6 space-y-4">
          <input type="hidden" name="next" value={nextPath} />
          <label className="block">
            <span className="mb-2 block text-xs font-black uppercase tracking-[0.2em] text-mutedInk">Password</span>
            <input
              autoFocus
              required
              name="password"
          ...
          {params.error ? <p className="text-sm font-bold text-coral">That password did not work. Try again.</p> : null}
